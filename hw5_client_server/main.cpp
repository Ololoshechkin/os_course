...
const std::string message = "abacaba";

int main(int argc, char** args) {
    if (strcmp(args[1], "server") == 0) {
        try {
            auto server = MessageServer(InetSocketAddress(kServerPort));
            server.Run();
        } catch (...) {
        }
    } else {
        try {
            ...
            > on_chat_requests = [&any_chat_requests, &comand_promise,
                                  &extra_string, &print_header](
                    const std::vector<
                            std::string
                    >& partner_names
            ) -> std::string {
            ...
            auto client = MessageClient(InetSocketAddress(kServerPort), args[2],
            ...
            while (true) {
                ...
                if (comand == "users") {
                    ...
                    if (client.StartChatWith(name)) {
                        ...
                        std::cout << "failed to open chat with user " << name
                                  << std::endl;
                    }
                    client.ContinueUpdates();
                } else if (comand == "exit_chat") {
                    if (client.state != MessageUnit::ClientState::IN_CHAT) {
                        std::cout << "client not belong to any chat"
                                  << std::endl;
                        continue;
                    }
                    client.StopUpdates();
                    ...
                    if (!client.SendChatMessage(message)) {
                        std::cout << "partner exited the current chat"
                                  << std::endl;
                        extra_string = "";
                    }
                    client.ContinueUpdates();
                    ...
                    if (client.state == MessageUnit::ClientState::AVAILABLE) {
                        std::cout << "partner disconnected from the chat"
                                  << std::endl;
                        extra_string = "";
                    }
