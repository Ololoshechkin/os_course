...
void
EventManager::SubscribeWrite(int fd, const std::function<void()>& handler) {
  ndfs = std::max(ndfs, fd + 1);
  FD_SET(fd, &write_fds);
...
void
EventManager::SubscribeError(int fd, const std::function<void()>& handler) {
  ndfs = std::max(ndfs, fd + 1);
  FD_SET(fd, &error_fds);
  fd_to_error_handler[fd] = handler;
}

void EventManager::SubscribeToSignal(
        int signal_id, const std::function<void()>& handler
) {
  struct sigaction action;
  memset(&action, 0, sizeof(action));
  action.sa_handler = [](int sig) {
    signal_set.insert(sig);
  };
...
void EventManager::Start() {
  while (true) {
    int res = pselect(
            ndfs, &read_fds, &write_fds, &error_fds, nullptr, &signal_mask);
    if (res < 0) {
      if (errno == EINTR) {
